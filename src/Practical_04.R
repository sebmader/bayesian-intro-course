...
ggplot(globalPlants, aes(z.lat, log(height))) +
  geom_point(alpha = 0.5)
ggplot(globalPlants, aes(z.rain, log(height))) +
  geom_point(alpha = 0.5)
...
# 3 categorical and continuous predictor -----------------------------------------
...
fit.ancova3.add <- brm(S ~ landscape + log.area.,
  prior = prior(normal(0, 10), class = b),
  prior = prior(normal(10, 10), class = b, coef = "log.area."),
  data = birds
)
fit.ancova3.int <- brm(S ~ landscape * log.area.,
  prior = prior(normal(0, 10), class = b),
  prior = prior(normal(10, 10), class = b, coef = "log.area."),
  data = birds
)

summary(fit.ancova3.add, prior = TRUE)
summary(fit.ancova3.int, prior = TRUE)


p3_add <- plot(
  conditional_effects(fit.ancova3.add),
  points = TRUE,
  point_args = c(alpha = 0.5),
  plot = FALSE
)
p3_add[[2]]
 + geom_line()
 + facet_wrap(var(landscape)) # doesn't work

p3_int <- plot(
  conditional_effects(fit.ancova3.int),
  points = TRUE,
  point_args = c(alpha = 0.5)
)

LOO(fit.ancova3.add, fit.ancova3.int)

pp_check(fit.ancova3.add, ndraws = 100)
pp_check(fit.ancova3.add, type = "scatter_avg")
check_model(fit.ancova3.add, check = c("linearity", "homogeneity", "qq", "normality"))