...
stan.data <- list(
  N = nrow(data),
  x = as.vector(scale(data$lat)), # convert to vector to get rid of
  #  additional infos from scale function
  y = log(data$height)
)

# Compiling code & MCMC with single function stan()

fit1 <- stan(file = here("src/lm.stan"), data = stan.data)
...
fit3 <- stan(file = here("src/lm_anova.stan"), data = stan.data)
...
fit_exercise <- stan(
  file = here("src/lm_ancova_exercise.stan"),
  data = stan_data
)

print(fit_exercise, probs = c(0.05, 0.95))
summary(fit_exercise, probs = c(0.05, 0.95))$summary |> round(3)
...
print(fit_exercise_2, probs = c(0.05, 0.95))
summary(fit_exercise_2, probs = c(0.05, 0.95))$summary |> round(3)

mcmc_combo(fit_exercise_2,
  combo = c("hist", "trace"),
  pars = c("b[1]", "b[2]", "b[3]", "b[4]", "a", "sigma")
)
